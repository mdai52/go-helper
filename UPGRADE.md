...
| `github.com/rehiy/pango/socket` | `github.com/rehiy/libgo/relay` | TCP 转发功能 |
...
relay.TCPRelay(ws, &relay.TCPParam{...})  // TcpRelayParam 重命名为 TCPParam
```
...
socket 包拆分为 websocket 和 relay，需手动处理：
...
- 使用 `TcpRelay`、`TcpRelayParam` → 改为 `relay` 包
...
| `socket.TcpRelayParam` | `relay.TCPParam` | relay |
...
socket 包已拆分，根据功能选择 websocket 或 relay。
...
4. **手动处理 socket**：根据实际使用拆分为 websocket 或 relay
5. **删除 recovery**：改用 httpd.Recovery() 中间件
6. **更新类型引用**：WsConn → Conn，PlainData → Message，TcpRelayParam → TCPParam
